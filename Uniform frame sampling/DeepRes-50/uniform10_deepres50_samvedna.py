...
# Original SAMVEDNA root used in the supplied code.
SAMVEDNA_VIDEO_DIR = Path(r"D:\All emotions\Video")
...
OUTPUT_DIR = Path(
    r"D:\SAMVEDNA_UNIFORM10\deepres50_outputs"
)
...
def infer_class_from_relative_path(relative_path: Path):
    """
    Infer the SAMVEDNA class from any parent emotion folder.
...
def make_video_id(
    ...
    """
    Build a stable source-video ID while removing the emotion folder.
...
def discover_samvedna_videos(
    ...
    """
    Find videos belonging to the five retained SAMVEDNA emotions.
    """
    video_root = Path(
        video_root
    )
    ...
    for video_path in videos:
        ...
        (
            matched_class_part,
            class_name,
        ) = infer_class_from_relative_path(
            relative_path
        )
    ...
    if not selected:
        raise RuntimeError(
            "No SAMVEDNA videos matched the retained five classes."
        )
...
def prepare_samvedna_uniform_dataset(
    ...
    Apply:
        ...
        -> facial preprocessing
    to every retained SAMVEDNA video.
    """
    videos = (
        discover_samvedna_videos(
            video_root
        )
    )
...
model.save(
    OUTPUT_DIR
    / "samvedna_uniform10_deepres50.keras"
)
...
plt.title(
    "DeepRes-50 Accuracy - SAMVEDNA Uniform Sampling"
)
...
plt.title(
    "DeepRes-50 Loss - SAMVEDNA Uniform Sampling"
)
