...
SAMVEDNA_VIDEO_DIR = Path(
    r"D:\All emotions\Video"
)

ALIGNED_FRAME_DIR = Path(
    r"D:\SAMVEDNA_UNIFORM10\aligned_frames"
)
...
OUTPUT_DIR = Path(
    r"D:\SAMVEDNA_UNIFORM10\attnvision_outputs"
)
...
# SAMVEDNA class discovery is based on emotion-folder names.
...
# =====================================================================
# 4. SAMVEDNA DATASET DISCOVERY
# =====================================================================

def infer_samvedna_class(relative_path: Path):
    """
    Infer one retained SAMVEDNA emotion class from parent folders.

    Supported folder-name variants include:
        Anger / angry
        Disgust / disgusted
        Fear / fearful
        Happy / happiness
    ...
    for part in relative_path.parts[:-1]:
        canonical = SAMVEDNA_CLASS_LOOKUP.get(
            part.lower()
        )
...
def make_video_id(
    ...
    """
    Build a stable SAMVEDNA source-video ID while removing the
...
def discover_samvedna_videos(
    video_root: Path,
):
    """
    Find SAMVEDNA videos belonging to the five retained emotion classes.
    """
    video_root = Path(
        video_root
    )
    ...
    if not video_root.exists():
        raise FileNotFoundError(
            "SAMVEDNA video directory does not exist:\n"
            f"{video_root}"
        )
...
def prepare_samvedna_uniform_dataset(
    video_root: Path,
):
    """
    SAMVEDNA:
        ...
        -> 48x48x3
    """
    videos = discover_samvedna_videos(
        video_root
    )
...
if RUN_FRAME_PREPARATION:
    prepare_samvedna_uniform_dataset(
        SAMVEDNA_VIDEO_DIR
    )
...
BEST_MODEL_PATH = (
    OUTPUT_DIR
    / "best_samvedna_uniform10_attnvision.pt"
)
...
plt.title(
    "AttnVision/ViT Accuracy - SAMVEDNA Uniform Sampling"
)
...
plt.title(
    "AttnVision/ViT Loss - SAMVEDNA Uniform Sampling"
)
...
plt.title(
    ...
    "SAMVEDNA Uniform Sampling Every 10th Frame"
)
