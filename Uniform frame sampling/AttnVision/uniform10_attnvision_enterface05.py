...
ALIGNED_FRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05_UNIFORM10\aligned_frames"
)
...
OUTPUT_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05_UNIFORM10\attnvision_outputs"
)
...
# eNTERFACE'05 class discovery is based on emotion-folder names.
# Surprise is intentionally excluded to preserve the five-class protocol.
...
# =====================================================================
# 4. eNTERFACE'05 DATASET DISCOVERY
# =====================================================================
...
def infer_enterface05_class(relative_path: Path):
    ...
    Typical layouts:
        subject_01/anger/.../video.avi
        subject_01/happiness/.../video.avi

    Surprise/unmatched videos are ignored because this study retains only
    Anger, Disgust, Fear, Happy, and Sad.
    """
    matches = []

    for part in relative_path.parts[:-1]:
        canonical = ENTERFACE05_CLASS_LOOKUP.get(
            part.lower()
        )
...
def make_video_id(
    ...
    """
    Build a stable eNTERFACE'05 source-video ID while removing the
...
def discover_enterface05_videos(
    video_root: Path,
):
    """
    Find eNTERFACE'05 videos belonging to the five retained emotion classes.
    """
    video_root = Path(
        video_root
    )
    ...
    if not video_root.exists():
        raise FileNotFoundError(
            "eNTERFACE'05 video directory does not exist:\n"
            f"{video_root}"
        )
...
def prepare_enterface05_uniform_dataset(
    video_root: Path,
):
    """
    eNTERFACE'05:
        ...
        -> 48x48x3
    """
    videos = discover_enterface05_videos(
        video_root
    )
...
if RUN_FRAME_PREPARATION:
    prepare_enterface05_uniform_dataset(
        ENTERFACE05_VIDEO_DIR
    )
...
BEST_MODEL_PATH = (
    OUTPUT_DIR
    / "best_enterface05_uniform10_attnvision.pt"
)
...
plt.title(
    "AttnVision/ViT Accuracy - eNTERFACE\'05 Uniform Sampling"
)
...
plt.title(
    "AttnVision/ViT Loss - eNTERFACE\'05 Uniform Sampling"
)
...
plt.title(
    ...
    "eNTERFACE\'05 Uniform Sampling Every 10th Frame"
)
