...
KEYFRAME_DIR = Path(r"C:\PATH\TO\PUMAVE-D_KEYFRAME\selected_keyframes")
ALIGNED_KEYFRAME_DIR = Path(r"C:\PATH\TO\PUMAVE-D_KEYFRAME\aligned_keyframes")
DLIB_LANDMARK_MODEL = Path(r"C:\PATH\TO\shape_predictor_68_face_landmarks.dat")
OUTPUT_DIR = Path(r"C:\PATH\TO\PUMAVE-D_KEYFRAME\deepres50_outputs")
...
# PUMAVE-D emotion folders may use common noun/adjective variants.
# Neutral is intentionally not mapped because the paper retains only
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer one retained PUMAVE-D emotion class from the parent folders.
    ...
    Neutral/unmatched videos are ignored because the five-class protocol
...
def discover_dataset_videos(video_root: Path):
    ...
    print(f"Ignored PUMAVE-D videos         : {len(ignored)}")
...
def prepare_keyframe_dataset(video_root: Path):
    """Run optimized one-pass preparation for all retained PUMAVE-D videos."""
...
if RUN_KEYFRAME_PIPELINE:
    prepare_keyframe_dataset(PUMAVED_VIDEO_DIR)
