...
ENTERFACE05_VIDEO_DIR = Path(r"C:\PATH\TO\eNTERFACE05\VIDEOS")
KEYFRAME_DIR = Path(r"C:\PATH\TO\eNTERFACE05_KEYFRAME\selected_keyframes")
ALIGNED_KEYFRAME_DIR = Path(r"C:\PATH\TO\eNTERFACE05_KEYFRAME\aligned_keyframes")
DLIB_LANDMARK_MODEL = Path(r"C:\PATH\TO\shape_predictor_68_face_landmarks.dat")
OUTPUT_DIR = Path(r"C:\PATH\TO\eNTERFACE05_KEYFRAME\deepres50_outputs")
...
# eNTERFACE'05 is commonly organized with emotion names in the
# directory path, including anger, disgust, fear, happiness, sadness,
# and surprise. Surprise is intentionally not mapped because the paper
# retains only Anger, Disgust, Fear, Happy, and Sad.
CLASS_LOOKUP = {
    "anger": "Anger",
    "angry": "Anger",
    "disgust": "Disgust",
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer one retained eNTERFACE'05 emotion class from the parent folders.

    Supports layouts such as:
        subject_01/happiness/.../video.avi

    The class is identified from the emotion-named directory in the path.
    Surprise is ignored because it is not part of the five-class protocol.
    """
    matches = []
    ...
    classes = {canonical for _, canonical in matches}
    if len(classes) > 1:
        raise ValueError(
            f"Ambiguous emotion folders in '{relative_path}': {sorted(classes)}"
        )
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """Create a stable eNTERFACE'05 video ID while removing the emotion folder.
...
def discover_dataset_videos(video_root: Path):
    ...
    for video_path in videos:
        ...
        matched_part, class_name = infer_class_from_relative_path(relative)

        if class_name is None:
            ignored.append(relative)
            continue

        video_id = make_video_id(relative, matched_part)
    ...
    print(f"Ignored eNTERFACE'05 videos   : {len(ignored)}")
...
def prepare_keyframe_dataset(video_root: Path):
    """Run optimized one-pass preparation for all retained eNTERFACE'05 videos."""
...
if RUN_KEYFRAME_PIPELINE:
    prepare_keyframe_dataset(ENTERFACE05_VIDEO_DIR)
