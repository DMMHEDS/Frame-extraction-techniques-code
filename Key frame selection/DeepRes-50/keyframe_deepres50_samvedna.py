...
KEYFRAME_DIR = Path(r"C:\PATH\TO\SAMVEDNA_KEYFRAME\selected_keyframes")
ALIGNED_KEYFRAME_DIR = Path(r"C:\PATH\TO\SAMVEDNA_KEYFRAME\aligned_keyframes")
DLIB_LANDMARK_MODEL = Path(r"C:\PATH\TO\shape_predictor_68_face_landmarks.dat")
OUTPUT_DIR = Path(r"C:\PATH\TO\SAMVEDNA_KEYFRAME\deepres50_outputs")
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer one retained emotion class from any parent folder."""
    matches = []
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """Create a stable source-video ID while removing the emotion folder."""
...
def discover_dataset_videos(video_root: Path):
    ...
    print(f"Ignored/unmatched videos        : {len(ignored)}")
...
def prepare_keyframe_dataset(video_root: Path):
    """Run optimized one-pass preparation for all retained SAMVEDNA videos."""
...
if RUN_KEYFRAME_PIPELINE:
    prepare_keyframe_dataset(SAMVEDNA_VIDEO_DIR)
