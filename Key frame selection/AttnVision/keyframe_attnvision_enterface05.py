...
ENTERFACE05_VIDEO_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05"
)

RAW_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05_KEYFRAME\selected_keyframes"
)

ALIGNED_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05_KEYFRAME\aligned_keyframes"
)
...
OUTPUT_DIR = Path(
    r"D:\IIT mandi\Facial datasets\eNTERFACE05_KEYFRAME\attnvision_outputs"
)
...
# eNTERFACE'05 commonly stores the emotion name in a parent folder.
# Surprise is intentionally excluded from the five-class protocol.
ENTERFACE05_CLASS_LOOKUP = {
    "anger": "Anger",
    "angry": "Anger",
    "disgust": "Disgust",
    "disgusted": "Disgust",
...
# ================================================================
# 4. eNTERFACE'05 DISCOVERY / LABEL PARSING
# ================================================================

def infer_enterface05_class(relative_path: Path):
    """
    Infer one retained eNTERFACE'05 emotion class from parent folders.

    Example:
        subject_01/happiness/.../video.avi
        -> Happy

    Surprise/unmatched videos are ignored because the paper retains only
    Anger, Disgust, Fear, Happy, and Sad.
    """
    matches = []

    for part in relative_path.parts[:-1]:
        canonical = ENTERFACE05_CLASS_LOOKUP.get(part.lower())
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """
    Create a stable eNTERFACE'05 source-video ID while removing the
    emotion folder.

    Example:
        subject_01/happiness/sentence_02/video.avi
        -> subject_01__sentence_02__video
    """
...
def discover_enterface05_videos(video_root: Path):
    """Find retained five-class eNTERFACE'05 videos."""
    video_root = Path(video_root)

    if not video_root.exists():
        raise FileNotFoundError(
            f"eNTERFACE'05 video directory does not exist:\n{video_root}"
        )
    ...
    for video_path in videos:
        ...
        matched_class_part, class_name = infer_enterface05_class(
            relative_path
        )
    ...
    if not selected:
        raise RuntimeError(
            "No eNTERFACE'05 videos matched the selected five-class protocol."
        )
...
def prepare_enterface05_keyframes(video_root: Path):
    videos = discover_enterface05_videos(video_root)
    total_selected = 0
...
if RUN_KEYFRAME_PIPELINE:
    prepare_enterface05_keyframes(ENTERFACE05_VIDEO_DIR)
...
best_model_path = OUTPUT_DIR / "best_enterface05_keyframe_attnvision.pt"
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("AttnVision/ViT Accuracy - eNTERFACE'05 Key Frames")
plt.legend()
plt.tight_layout()
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("AttnVision/ViT Loss - eNTERFACE'05 Key Frames")
plt.legend()
plt.tight_layout()
