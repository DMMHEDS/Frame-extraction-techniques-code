...
SAMVEDNA_VIDEO_DIR = Path(
    r"D:\IIT mandi\Facial datasets\SAMVEDNA"
)

RAW_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\SAMVEDNA_KEYFRAME\selected_keyframes"
)

ALIGNED_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\SAMVEDNA_KEYFRAME\aligned_keyframes"
)
...
OUTPUT_DIR = Path(
    r"D:\IIT mandi\Facial datasets\SAMVEDNA_KEYFRAME\attnvision_outputs"
)
...
# SAMVEDNA class discovery is path-based.
# Only the five classes used in the paper are retained.
# Common noun/adjective folder-name variants are supported.
SAMVEDNA_CLASS_LOOKUP = {
    "anger": "Anger",
    "angry": "Anger",
    "disgust": "Disgust",
    "disgusted": "Disgust",
...
# ================================================================
# 4. SAMVEDNA DISCOVERY / LABEL PARSING
# ================================================================

def infer_samvedna_class(relative_path: Path):
    """
    Infer one retained SAMVEDNA emotion class from the video's parent folders.

    Supported layouts include, for example:
        Anger/video01.mp4
        Subject01/Anger/video01.mp4

    Only Anger, Disgust, Fear, Happy, and Sad are retained.
    """
    matches = []

    for part in relative_path.parts[:-1]:
        canonical = SAMVEDNA_CLASS_LOOKUP.get(part.lower())
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """
    Create a stable SAMVEDNA source-video ID while removing the emotion folder.
...
def discover_samvedna_videos(video_root: Path):
    """Find retained five-class SAMVEDNA videos."""
    video_root = Path(video_root)

    if not video_root.exists():
        raise FileNotFoundError(
            f"SAMVEDNA video directory does not exist:\n{video_root}"
        )
    ...
    for video_path in videos:
        ...
        matched_class_part, class_name = infer_samvedna_class(
            relative_path
        )
    ...
    if not selected:
        raise RuntimeError(
            "No SAMVEDNA videos matched the selected five-class protocol. "
            "This implementation expects the emotion label to appear in a "
            "parent folder. If your SAMVEDNA copy uses a different naming "
            "scheme, update infer_samvedna_class()."
        )
...
def prepare_samvedna_keyframes(video_root: Path):
    videos = discover_samvedna_videos(video_root)
    total_selected = 0
...
if RUN_KEYFRAME_PIPELINE:
    prepare_samvedna_keyframes(SAMVEDNA_VIDEO_DIR)
...
best_model_path = OUTPUT_DIR / "best_samvedna_keyframe_attnvision.pt"
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("AttnVision/ViT Accuracy - SAMVEDNA Key Frames")
plt.legend()
plt.tight_layout()
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("AttnVision/ViT Loss - SAMVEDNA Key Frames")
plt.legend()
plt.tight_layout()
