...
RAW_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\RAVDESS_KEYFRAME\selected_keyframes"
)

ALIGNED_KEYFRAME_DIR = Path(
    r"D:\IIT mandi\Facial datasets\RAVDESS_KEYFRAME\aligned_keyframes"
)
...
OUTPUT_DIR = Path(
    r"D:\IIT mandi\Facial datasets\RAVDESS_KEYFRAME\attnvision_outputs"
)

MODEL_NAME = "dima806/face_emotions_image_detection"

# RAVDESS modality 02 = video-only.
...
# ================================================================
# 4. RAVDESS DISCOVERY / LABEL PARSING
# ================================================================
...
def parse_ravdess_filename(video_path: Path):
    ...
    class_name = RAVDESS_EMOTION_CODE_TO_CLASS.get(emotion_code)
...
def discover_ravdess_videos(video_root: Path):
    ...
    for video_path in videos:
        class_name, modality = parse_ravdess_filename(video_path)
        ...
        selected.append(
            (video_path, relative_path, class_name, video_id)
        )
    ...
    print("Skipped by modality      :", skipped_modality)
    print("Skipped other emotions   :", skipped_emotion)
    print("Malformed filenames      :", malformed)
...
def prepare_ravdess_keyframes(video_root: Path):
    videos = discover_ravdess_videos(video_root)
    total_selected = 0
...
if RUN_KEYFRAME_PIPELINE:
    prepare_ravdess_keyframes(RAVDESS_VIDEO_DIR)
...
best_model_path = OUTPUT_DIR / "best_ravdess_keyframe_attnvision.pt"
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("AttnVision/ViT Accuracy - RAVDESS Key Frames")
plt.legend()
plt.tight_layout()
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("AttnVision/ViT Loss - RAVDESS Key Frames")
plt.legend()
plt.tight_layout()
