...
EXTRACTED_IFRAME_DIR = Path(r"C:\PATH\TO\RAVDESS_IFRAME\extracted_frames")
ALIGNED_FRAME_DIR = Path(r"C:\PATH\TO\RAVDESS_IFRAME\aligned_frames")
DLIB_LANDMARK_MODEL = Path(r"C:\PATH\TO\shape_predictor_68_face_landmarks.dat")
OUTPUT_DIR = Path(r"C:\PATH\TO\RAVDESS_IFRAME\vit_checkpoints")
...
# RAVDESS filenames encode a 7-part numerical identifier, e.g.:
#     02-01-06-01-02-01-12.mp4
#     modality-vocalChannel-emotion-intensity-statement-repetition-actor
# See: Livingstone & Russo (2018), PLoS ONE 13(5): e0196391.
RAVDESS_EMOTION_CODE_TO_NAME = {
    "01": "neutral",
    "02": "calm",
    "03": "happy",
    "04": "sad",
...
# Map RAVDESS's 8 emotions onto the 5 retained classes.
# "neutral", "calm", and "surprised" are deliberately excluded, matching
# the five-class scheme used throughout this pipeline.
RAVDESS_EMOTION_TO_CLASS = {
    "angry": "Anger",
    "disgust": "Disgust",
    "fearful": "Fear",
    "happy": "Happy",
...
def infer_class_from_filename(video_path: Path):
    """
    Parse a RAVDESS filename's 7-part numerical identifier and map its
    emotion field onto one of the five retained classes.

    Expected filename pattern (dash-separated, no other dashes allowed):
        modality-vocalChannel-emotion-intensity-statement-repetition-actor
        e.g. 02-01-06-01-02-01-12.mp4  ->  emotion code "06" -> "fearful" -> "Fear"

    Returns None if the filename doesn't match the 7-field convention, or
    if its emotion (neutral / calm / surprised) isn't one of the five
    retained classes.
    """
    ...
    emotion_name = RAVDESS_EMOTION_CODE_TO_NAME.get(emotion_code)

    if emotion_name is None:
        return None

    return RAVDESS_EMOTION_TO_CLASS.get(emotion_name)


def make_video_id(video_path: Path):
    """
    RAVDESS filenames (minus extension) are already a unique identifier
    across the whole dataset -- the actor field alone guarantees no two
    source videos share a stem -- so no extra path-based ID is needed.
    """
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    ...
    if not video_root.exists():
        raise FileNotFoundError(
            f"RAVDESS video directory was not found:\n{video_root}\nUpdate RAVDESS_VIDEO_DIR."
        )
    ...
    for video_path in all_video_files:
        ...
        class_name = infer_class_from_filename(video_path)
    ...
    if not selected_videos:
        raise RuntimeError(
            ...
            "Check that filenames follow the RAVDESS 7-field convention "
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(
        video_root=RAVDESS_VIDEO_DIR, output_root=EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES
    )
