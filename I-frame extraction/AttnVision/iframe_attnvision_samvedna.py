...
OUTPUT_DIR = Path(r"C:\PATH\TO\SAMVEDNA_IFRAME\vit_checkpoints")
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    ...
    if not video_root.exists():
        raise FileNotFoundError(
            f"SAMVEDNA video directory was not found:\n{video_root}\nUpdate SAMVEDNA_VIDEO_DIR."
        )
    ...
    if not selected_videos:
        raise RuntimeError(
            ...
            "Check the SAMVEDNA folder layout."
        )
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(
        video_root=SAMVEDNA_VIDEO_DIR, output_root=EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES
    )
