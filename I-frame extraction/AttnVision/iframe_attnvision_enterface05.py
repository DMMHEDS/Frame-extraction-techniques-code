...
OUTPUT_DIR = Path(r"C:\PATH\TO\eNTERFACE05_IFRAME\vit_checkpoints")
...
# eNTERFACE'05 emotion folders commonly include:
# anger, disgust, fear, happiness, sadness, surprise.
# Only the five common emotions used in the paper are retained.
CLASS_LOOKUP = {
    "anger": "Anger",
    "disgust": "Disgust",
    "fear": "Fear",
...
def infer_class_from_relative_path(relative_path: Path):
    """
    Infer one of the five retained eNTERFACE'05 emotion classes from
    the video's parent folders.

    Typical organization is subject/emotion/.../video.
    Surprise is ignored because the paper evaluates only
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """
    Build a unique eNTERFACE'05 video ID while removing the emotion folder.
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    ...
    if not video_root.exists():
        raise FileNotFoundError(
            f"eNTERFACE\'05 video directory was not found:\n{video_root}\nUpdate ENTERFACE05_VIDEO_DIR."
        )
    ...
    if not selected_videos:
        raise RuntimeError(
            ...
            "Check the eNTERFACE\'05 folder layout."
        )
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(
        video_root=ENTERFACE05_VIDEO_DIR, output_root=EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES
    )
