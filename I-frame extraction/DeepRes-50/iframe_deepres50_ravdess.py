...
# RAVDESS emotion code is the third field in the official filename:
# 01 = neutral, 02 = calm, 03 = happy, 04 = sad,
# 05 = angry, 06 = fearful, 07 = disgust, 08 = surprised.
# Only the five emotion classes retained in the paper are used.
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer the retained emotion class from an official RAVDESS filename.

    Example:
        01-01-05-01-01-01-01.mp4
                 ^^
                 emotion code 05 -> Anger

    Neutral (01), calm (02), and surprised (08) clips are skipped because
    the paper evaluates only Anger, Disgust, Fear, Happy, and Sad.
    """
...
def make_video_id(relative_path: Path):
    """Create a unique video ID from the RAVDESS relative path.
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    """Walks the RAVDESS root, parses the emotion code from each official
    filename, and pulls I-frames for videos in the 5 retained classes. Output ends up as
    output_root/<class>/<video_id>/iframe_00001.png etc."""
    video_root = Path(video_root)
    output_root = Path(output_root)

    if not video_root.exists():
        raise FileNotFoundError(f"video folder not found: {video_root} — check RAVDESS_VIDEO_DIR")
    ...
    for video_path in all_video_files:
        ...
        class_name = infer_class_from_relative_path(relative_path)
    ...
    print(f"RAVDESS videos in the 5 retained classes: {len(selected_videos)}")
    print(f"skipped RAVDESS clips (neutral/calm/surprised or invalid name): {len(ignored_videos)}")
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(RAVDESS_VIDEO_DIR, EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES)
