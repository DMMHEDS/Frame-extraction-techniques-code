...
# PUMAVE-D visual emotion labels include Anger, Disgust, Fear,
# Happy, Neutral, and Sad. Neutral is intentionally omitted because
# this paper evaluates only the five common emotions.
# Common noun/adjective folder-name variants are accepted.
CLASS_LOOKUP = {
    "anger": "Anger",
    "angry": "Anger",
    "disgust": "Disgust",
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer one of the five retained PUMAVE-D emotion classes from the path.

    The function searches all parent folders for an emotion label, so it
    supports layouts such as:
        Anger/video01.mp4
    or:
        Subject01/Anger/video01.mp4

    Neutral clips are intentionally skipped because the paper evaluates only
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """Build a unique PUMAVE-D video ID with the emotion folder removed.
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    ...
    if not video_root.exists():
        raise FileNotFoundError(f"video folder not found: {video_root} — check PUMAVED_VIDEO_DIR")
    ...
    print(f"PUMAVE-D videos in the 5 retained classes: {len(selected_videos)}")
    print(f"skipped PUMAVE-D clips (neutral/unmatched): {len(ignored_videos)}")
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(PUMAVED_VIDEO_DIR, EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES)
