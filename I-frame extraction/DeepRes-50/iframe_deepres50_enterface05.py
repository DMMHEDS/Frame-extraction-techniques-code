...
EXTRACTED_IFRAME_DIR = Path(r"C:\PATH\TO\eNTERFACE05_IFRAME\extracted_frames")
ALIGNED_FRAME_DIR = Path(r"C:\PATH\TO\eNTERFACE05_IFRAME\aligned_frames")
...
# eNTERFACE'05 uses folder labels such as "happiness" and "sadness".
# Surprise and neutral are intentionally omitted because this paper
# evaluates only the five emotions common to all four datasets.
CLASS_LOOKUP = {
    "anger": "Anger",
    "disgust": "Disgust",
    "fear": "Fear",
...
def infer_class_from_relative_path(relative_path: Path):
    """Infer one of the five retained classes from the eNTERFACE'05 path.

    The original database is commonly organized as:
        subject i / emotion / sentence i / video.avi

    Emotion folders include anger, disgust, fear, happiness, sadness,
    surprise and neutral. Only Anger, Disgust, Fear, Happy and Sad are
    retained here; surprise and neutral are skipped."""
    matches = []
...
def make_video_id(relative_path: Path, matched_class_part: str):
    """Build a unique eNTERFACE'05 video ID with the emotion folder removed.

    Example:
        subject 1/happiness/sentence 2/video.avi
        -> subject 1__sentence 2__video
    """
...
def extract_iframes_from_dataset(video_root: Path, output_root: Path, overwrite: bool = False):
    ...
    if not video_root.exists():
        raise FileNotFoundError(f"video folder not found: {video_root} — check ENTERFACE05_VIDEO_DIR")
    ...
    print(f"eNTERFACE\'05 videos in the 5 retained classes: {len(selected_videos)}")
    print(f"skipped eNTERFACE\'05 clips (surprise/neutral/unmatched): {len(ignored_videos)}")
...
if RUN_IFRAME_EXTRACTION:
    extract_iframes_from_dataset(ENTERFACE05_VIDEO_DIR, EXTRACTED_IFRAME_DIR, overwrite=OVERWRITE_EXISTING_IFRAMES)
